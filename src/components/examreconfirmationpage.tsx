...
export default function App() {
  ...
  const examData = {
    title: 'CISF Constable (Fireman): General Awareness - सामान्य जागरूकता - Quiz',
    duration: '5 Mins',
    maxMarks: '5',
    instructions: [
    ...
    agreementText: "I have read all the instructions carefully and have understood them. I agree not to cheat or use unfair means in this examination. I understand that using unfair means of any sort for my own or someone else's advantage will lead to my immediate disqualification. The decision of Testbook.com will be final in these matters and cannot be appealed."
  };
...
const ExamConfirmationPage = ({ examDetails }) => {
  ...
  return (
    <div className="bg-gray-900 min-h-screen font-sans text-gray-200 p-4 sm:p-6 lg:p-8">
      <div className="max-w-4xl mx-auto"> {/* Adjusted max-width for a more centered, single-column layout */}
        {/* Top Header */}
        <header className="flex justify-between items-center mb-6">
          <h1 className="text-lg md:text-xl font-semibold text-gray-100">{examDetails.title}</h1>
        ...
        <div className="grid grid-cols-1 gap-8">
          ...
          <main className="bg-gray-800 p-6 sm:p-8 rounded-xl shadow-lg">
            <div className="mb-6">
              <p className="text-sm text-gray-400">Duration: {examDetails.duration}</p>
              <h2 className="text-xl font-bold mt-2 text-white">Read the following instructions carefully.</h2>
            </div>
            ...
            <div className="flex items-start space-x-4">
              ...
              />
              <label htmlFor="confirmation" className="text-sm text-gray-400 cursor-pointer">
        ...
        <footer className="mt-8 pt-6 border-t border-gray-700 flex justify-between items-center">
          <button className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-6 rounded-lg transition-colors">
            Previous
          </button>
          <button
