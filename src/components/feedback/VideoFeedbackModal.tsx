...
const VideoFeedbackModal: React.FC<VideoFeedbackModalProps> = ({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4 pointer-events-auto"
      onClick={handleBackdropClick}
    >
      <div className="relative w-full max-w-xl max-h-[90vh] overflow-y-auto bg-white dark:bg-gradient-to-br dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 pointer-events-auto m-4 contrast-more:border-2 motion-reduce:animate-none animate-[fadeIn_0.5s_ease-out]">
        {/* Header */}
              ...
              {[1, 2, 3, 4, 5].map((ratingValue) => (
                ...
                >
                  <button
                    ...
                    className="cursor-pointer p-1 rounded-full transition-transform duration-200 ease-in-out enabled:hover:scale-110 motion-reduce:transition-none motion-reduce:hover:transform-none"
                  >
                    <SmileyIcon
                      rating={ratingValue}
                      active={(hoverRating ?? rating ?? 0) >= ratingValue}
                    />
                  </button>
                  <span
                    className={`text-xs text-center text-gray-500 dark:text-gray-400 h-4 transition-opacity ${
                      ![1, 5].includes(ratingValue)
                ...
                <div className="space-y-4 pt-2">
                  ...
                  <div className="relative">
                    <textarea
                      ...
                      rows={3}
                      maxLength={4000}
                      className={`w-full p-3 bg-gray-50 dark:bg-gray-900/50 border rounded-lg focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors resize-none ${
                        comment.length > 3800
